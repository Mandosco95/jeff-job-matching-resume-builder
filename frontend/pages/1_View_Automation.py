        ...
        if unread_response.status_code == 200 and read_response.status_code == 200:
            ...
            with tab1:
                if unread_jobs_data["jobs"]:
                    display_jobs(unread_jobs_data, "unread")
                else:
                    st.info("No unread jobs found.")
            
            # Display read jobs in the second tab
            with tab2:
                if read_jobs_data["jobs"]:
...
def display_jobs(jobs_data, tab_type):
    """Helper function to display jobs in a tab"""
    ...
    selected_company = st.selectbox(
        "Filter by Company",
        company_names,
        key=f"company_filter_{tab_type}"
    )
    ...
    # Display total count of filtered jobs for the current tab
    st.markdown(f"<h4 style='font-size: 14px;'>Found {len(jobs)} {tab_type.capitalize()} Jobs</h4>", unsafe_allow_html=True)
    
    # Display jobs in a clean format
    if not jobs:
        st.info(f"No {tab_type} jobs found for {selected_company}.")
    else:
        for job in jobs:
            with st.container():
                        ...
                        if tab_type == "unread":
                            ...
                            if st.button("Mark as Unread", key=f"mark_unread_{job.get('_id', '')}"):
                                try:
                                    ...
                                    if response.status_code == 200:
                                        st.success("Job marked as unread!")
                                        st.rerun()
                                    else:
                                        st.error("Failed to mark job as unread")
                                except Exception as e:
                                    st.error(f"Error marking job as unread: {str(e)}")
