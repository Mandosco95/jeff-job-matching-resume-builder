...
def display_jobs(jobs_data):
    ...
    st.markdown(f"<h4 style='font-size: 14px;'>Found {len(jobs)} Read Jobs</h4>", unsafe_allow_html=True)
    
    # Display jobs in a clean format
    if not jobs:
        st.info(f"No read jobs found for {selected_company}.")
    else:
        for job in jobs:
            with st.container():
...
# Main execution
try:
    with st.spinner("Fetching read jobs..."):
        ...
        if read_response.status_code == 200:
            ...
            if read_jobs_data["jobs"]:
                display_jobs(read_jobs_data)
            else:
                st.info("No read jobs found.")
        else:
            st.error(f"Error fetching read jobs: {read_response.text}")
except Exception as e:
